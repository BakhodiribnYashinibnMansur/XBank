...
func (s *TransferSaga) Execute(ctx context.Context, tr *transfer.Transfer) *Result {
	sagaID := GenerateID()
		...
		},
		{
			...
			Execute: func(ctx context.Context) error {
				return s.txManager.WithTx(ctx, func(txCtx context.Context) error {
					...
					money, _ := domain.NewMoney(amount, acc.Balance.Currency)
	...
	return Execute(ctx, sagaID, steps)
}
