...
var (
	ErrContactNotFound = domain.NewDomainError("CONTACT_NOT_FOUND", "contact not found")
	ErrContactExists   = domain.NewDomainError("CONTACT_EXISTS", "contact already exists")
	ErrContactSelf     = domain.NewDomainError("CONTACT_SELF", "cannot add yourself as a contact")
)
...
func NewContact(ownerID, contactID, customName string) (*Contact, error) {
	if ownerID == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "owner_id is required")
	}
	if contactID == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "contact_id is required")
	}
