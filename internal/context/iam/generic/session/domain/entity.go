...
var (
	ErrSessionNotFound = domain.NewDomainError("SESSION_NOT_FOUND", "session not found")
	ErrSessionExpired  = domain.NewDomainError("SESSION_EXPIRED", "session has expired")
	ErrInvalidToken    = domain.NewDomainError("INVALID_TOKEN", "invalid token")
)
...
func NewSession(userID, refreshTokenHash, userAgent, ipAddress string, expiresAt time.Time) (*Session, error) {
	if userID == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "user_id cannot be empty")
	}
	if refreshTokenHash == "" {
