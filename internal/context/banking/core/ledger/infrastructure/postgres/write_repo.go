...
func (r *WriteRepo) CreatePair(ctx context.Context, debit, credit *domain.Entry) error {
	start := time.Now()
...
func (r *WriteRepo) ListByAccountID(ctx context.Context, accountID string, limit, offset int) ([]*domain.Entry, error) {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
...
func (r *WriteRepo) BalanceByAccountID(ctx context.Context, accountID string) (int64, error) {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
	var balance int64
	err := db.QueryRow(ctx,
