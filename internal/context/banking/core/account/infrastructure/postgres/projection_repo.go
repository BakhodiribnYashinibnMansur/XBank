...
// ProjectionRepo - manages the CQRS read projection.
...
func (r *ProjectionRepo) Upsert(ctx context.Context, acc *account.Account, userEmail string) error {
	start := time.Now()
...
// AddCredit - increment total_credited.
func (r *ProjectionRepo) AddCredit(ctx context.Context, accountID string, amount int64) error {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
...
// AddDebit - increment total_debited.
func (r *ProjectionRepo) AddDebit(ctx context.Context, accountID string, amount int64) error {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
...
func (r *ProjectionRepo) GetSummary(ctx context.Context, accountID string) (*AccountSummary, error) {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
...
func (r *ProjectionRepo) ListByUserID(ctx context.Context, userID string, limit, offset int) ([]*AccountSummary, int64, error) {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
