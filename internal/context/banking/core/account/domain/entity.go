...
var (
	ErrAccountNotFound = domain.NewDomainError("ACCOUNT_NOT_FOUND", "account not found")
	ErrAccountFrozen   = domain.NewDomainError("ACCOUNT_FROZEN", "account is frozen")
	ErrAccountClosed   = domain.NewDomainError("ACCOUNT_CLOSED", "account is closed")
	ErrBalanceNotZero  = domain.NewDomainError("BALANCE_NOT_ZERO", "account balance must be 0 to close")
	ErrMissingUserID   = domain.NewDomainError("MISSING_FIELD", "user_id cannot be empty")
)
...
func NewAccount(userID string, currency domain.Currency) (*Account, error) {
	if userID == "" {
...
func LoadFromSnapshot(snap SnapshotState, version int, events []Event) *Account {
	a := &Account{
		...
		Balance:       domain.Money{Amount: snap.Balance, Currency: domain.Currency(snap.Currency)},
...
func (a *Account) Deposit(amount domain.Money) error {
...
func (a *Account) Withdraw(amount domain.Money) error {
	if err := a.checkActive(); err != nil {
		return err
	}
	if a.Balance.Currency != amount.Currency {
...
func (a *Account) Apply(e Event) {
	...
	case AccountOpenedData:
		...
		a.Balance = domain.Money{Amount: 0, Currency: domain.Currency(data.Currency)}
