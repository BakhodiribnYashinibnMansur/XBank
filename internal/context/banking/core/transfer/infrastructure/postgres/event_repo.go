...
func (r *EventRepo) LoadEvents(ctx context.Context, aggregateID string) ([]transfer.Event, error) {
	return r.loadEvents(ctx, aggregateID, 0)
}

// LoadEventsFromVersion - load events after a given version
func (r *EventRepo) LoadEventsFromVersion(ctx context.Context, aggregateID string, fromVersion int) ([]transfer.Event, error) {
	return r.loadEvents(ctx, aggregateID, fromVersion)
}

func (r *EventRepo) loadEvents(ctx context.Context, aggregateID string, fromVersion int) ([]transfer.Event, error) {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
...
func (r *EventRepo) SaveSnapshot(ctx context.Context, snapshot transfer.Snapshot) error {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
...
func (r *EventRepo) LoadSnapshot(ctx context.Context, aggregateID string) (*transfer.Snapshot, error) {
	start := time.Now()
	db := sharedpg.ExtractDBTX(ctx, r.pool)
