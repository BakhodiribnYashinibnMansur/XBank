...
func TestSchedule_Success(t *testing.T) {
	...
	st, err := svc.Schedule(context.Background(), "user-1", "acc-1", "acc-2", 5000, shared.Currency("UZS"), "test", executeAt)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSchedule_PastDate(t *testing.T) {
	...
	_, err := svc.Schedule(context.Background(), "user-1", "acc-1", "acc-2", 5000, shared.Currency("UZS"), "test", executeAt)
	if err == nil {
...
func TestSchedule_SameAccount(t *testing.T) {
	...
	executeAt := time.Now().Add(24 * time.Hour)
	_, err := svc.Schedule(context.Background(), "user-1", "acc-1", "acc-1", 5000, shared.Currency("UZS"), "test", executeAt)
	if err == nil {
...
func TestCancel_Success(t *testing.T) {
	...
	executeAt := time.Now().Add(24 * time.Hour)
	st, _ := svc.Schedule(context.Background(), "user-1", "acc-1", "acc-2", 5000, shared.Currency("UZS"), "test", executeAt)
