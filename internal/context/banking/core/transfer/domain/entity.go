...
var (
	ErrTransferNotFound = domain.NewDomainError("TRANSFER_NOT_FOUND", "transfer not found")
	ErrSameAccount      = domain.NewDomainError("SAME_ACCOUNT", "cannot transfer to the same account")
	ErrInvalidAmount    = domain.NewDomainError("INVALID_AMOUNT", "amount must be greater than zero")
	ErrTransferFailed   = domain.NewDomainError("TRANSFER_FAILED", "transfer processing failed")
)
...
func NewTransfer(fromAccountID, toAccountID string, amount domain.Money, description string) (*Transfer, error) {
...
func (t *Transfer) Apply(e Event) {
	...
	case TransferCreatedData:
		...
		t.Amount = domain.Money{Amount: data.Amount, Currency: domain.Currency(data.Currency)}
...
func LoadFromSnapshot(snap SnapshotState, version int, events []Event) *Transfer {
	t := &Transfer{
		...
		Amount:        domain.Money{Amount: snap.Amount, Currency: domain.Currency(snap.Currency)},
