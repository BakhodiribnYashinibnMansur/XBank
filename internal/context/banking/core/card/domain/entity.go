...
var (
	...
	ErrCardBlocked         = domain.NewDomainError("CARD_BLOCKED", "card is blocked")
	ErrCardExpired         = domain.NewDomainError("CARD_EXPIRED", "card has expired")
	ErrInvalidPIN          = domain.NewDomainError("INVALID_PIN", "invalid PIN")
	ErrPINAttemptsExceeded = domain.NewDomainError("PIN_ATTEMPTS_EXCEEDED", "card locked: too many wrong PIN attempts")
	ErrCardValidation      = domain.NewDomainError("CARD_VALIDATION", "card validation error")
)
...
func NewCard(accountID string, cardType Type) (*Card, error) {
	if accountID == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "account_id cannot be empty")
	}
...
func (c *Card) Block() error {
	if c.Status == StatusCancelled {
		return domain.NewDomainError("CARD_VALIDATION", "cannot block a cancelled card")
	}
	c.Status = StatusBlocked
	c.UpdatedAt = time.Now()
	return nil
...
func (c *Card) Unblock() error {
	if c.Status != StatusBlocked {
		return domain.NewDomainError("CARD_VALIDATION", "card is not blocked")
	}
	c.Status = StatusActive
	c.PINAttempts = 0
	c.UpdatedAt = time.Now()
...
func (c *Card) checkUsable() error {
	switch c.Status {
	case StatusInactive:
		return domain.NewDomainError("CARD_VALIDATION", "card is not activated")
	case StatusBlocked:
		return ErrCardBlocked
	case StatusExpired:
		return ErrCardExpired
	case StatusCancelled:
		return domain.NewDomainError("CARD_VALIDATION", "card is cancelled")
	}
	if c.IsExpired() {
