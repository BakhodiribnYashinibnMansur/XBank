...
var (
	ErrKYCRequired = domain.NewDomainError("KYC_REQUIRED", "KYC verification required")
	ErrKYCPending  = domain.NewDomainError("KYC_PENDING", "KYC verification is pending")
)
...
func NewVerification(userID string, docType DocType, docNumber, firstName, lastName, dob string) (*Verification, error) {
	if userID == "" || docNumber == "" || firstName == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "user_id, document_number and first_name are required")
	}
