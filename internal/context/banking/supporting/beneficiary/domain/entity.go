...
var (
	ErrBeneficiaryNotFound = domain.NewDomainError("BENEFICIARY_NOT_FOUND", "beneficiary not found")
	ErrBeneficiaryExists   = domain.NewDomainError("BENEFICIARY_EXISTS", "beneficiary already exists")
)
...
func NewBeneficiary(userID, name, accountNumber, bankName, bankCode, currency string, benType Type) (*Beneficiary, error) {
	if userID == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "user_id is required")
	}
	if name == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "name is required")
	}
	if accountNumber == "" {
		return nil, domain.NewDomainError("MISSING_FIELD", "account_number is required")
	}
