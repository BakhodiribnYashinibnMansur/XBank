...
func registerAdminRoutes(admin fiber.Router, bcs *DDDBoundedContexts) {
	...
	// Dashboard BC — admin analytics
	if bcs.Dashboard != nil {
		bcs.Dashboard.Handler.RegisterRoutes(admin)
	}

	// Currency BC — currency catalog
	if bcs.Currency != nil {
		bcs.Currency.Handler.RegisterRoutes(admin)
	}

	// Template BC — message templates
	if bcs.Template != nil {
		bcs.Template.Handler.RegisterRoutes(admin)
	}

	// Healthcheck BC — system health
	if bcs.Healthcheck != nil {
		bcs.Healthcheck.Handler.RegisterRoutes(admin)
	}
}
