...
func NewScheduledTransferRepository(pool *pgxpool.Pool) *ScheduledTransferRepository {
	return &ScheduledTransferRepository{pool: pool}
}

func (r *ScheduledTransferRepository) Create(ctx context.Context, st *transfer.ScheduledTransfer) error {
	start := time.Now()
...
func (r *ScheduledTransferRepository) GetByID(ctx context.Context, id string) (*transfer.ScheduledTransfer, error) {
	start := time.Now()
	...
	st.Amount.Currency = shared.Currency(currency)
	return st, nil
}

func (r *ScheduledTransferRepository) ListByUserID(ctx context.Context, userID string, limit, offset int) ([]*transfer.ScheduledTransfer, int64, error) {
	start := time.Now()
...
func (r *ScheduledTransferRepository) Update(ctx context.Context, st *transfer.ScheduledTransfer) error {
	start := time.Now()
	_, err := r.pool.Exec(ctx,
...
func (r *ScheduledTransferRepository) FetchDue(ctx context.Context, limit int) ([]*transfer.ScheduledTransfer, error) {
	start := time.Now()
	rows, err := r.pool.Query(ctx,
		`SELECT id, user_id, from_account_id, to_account_id, amount, currency, description,
		        status, execute_at, transfer_id, failure_reason, created_at, executed_at
